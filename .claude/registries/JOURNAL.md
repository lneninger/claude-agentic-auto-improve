...
- **`app:admin-panel` / `app:scalping-machine`** — optional sub-tag when a `ui-evolution` lesson is scope-specific (e.g., admin-panel's dark theme demands `-400` text where scalping-machine's light theme uses `-700`). Lessons without an `app:*` tag apply to both apps.
