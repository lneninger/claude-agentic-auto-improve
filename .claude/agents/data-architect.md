...
6. **Pre-write the implementer handoff.** Every contract ends with one or more pre-written TASK blocks — one per implementer agent that will be invoked after approval. The `Files to touch` list is load-bearing: the `concept-gate.py` hook reads it to allow edits.
